...
import {
     ...
     decrementTimer, incrementTabSwitch, addSuspiciousLog,
     setShowWarning, submitExam, resetExam,
} from '../../store/slices/examSlice'
import { testAPI, questionAPI, attemptAPI } from '../../services/api'
...
export default function ExamInterface() {
     ...
     const {
          ...
          tabSwitchCount, suspiciousLogs, showWarning, result,
     ...
     // Anti-cheat: tab switch / visibility / keyboard / clipboard
     useEffect(() => {
          if (!isStarted || isSubmitted) return
          ...
          const handleFullscreenChange = () => {
               if (!document.fullscreenElement) {
                    dispatch(addSuspiciousLog({ type: 'fullscreen_exit' }))
               }
          }

          const handleKeyDown = (e) => {
               if (
                    (e.ctrlKey && (e.key === 'c' || e.key === 'v' || e.key === 'a' || e.key === 'u' || e.key === 'p')) ||
                    (e.altKey && e.key === 'Tab') ||
                    e.key === 'F12' ||
                    (e.ctrlKey && e.shiftKey && (e.key === 'I' || e.key === 'J'))
               ) {
                    e.preventDefault()
                    dispatch(addSuspiciousLog({ type: 'blocked_shortcut', detail: `${e.ctrlKey ? 'Ctrl+' : ''}${e.altKey ? 'Alt+' : ''}${e.key}` }))
               }
          }

          const handleContextMenu = (e) => {
               e.preventDefault()
               dispatch(addSuspiciousLog({ type: 'right_click_blocked' }))
          }
     ...
     // Show warning on first tab switch, auto-submit on second
     useEffect(() => {
          if (!isStarted || isSubmitted || tabSwitchCount === 0) return
          if (tabSwitchCount === 1) {
               dispatch(setShowWarning(true))
               toast.error('⚠️ Warning: Tab switch detected! Next switch will auto-submit your test.')
          } else if (tabSwitchCount >= 2) {
               toast.error('❌ Test auto-submitted due to tab switching.')
               submitRef.current?.(true)
          }
     }, [tabSwitchCount, isStarted, isSubmitted, dispatch])
     ...
     if (loading || submittingState) return <PageLoader />
          ...
          return (
               <div className="min-h-screen bg-surface-50 dark:bg-surface-950 flex items-center justify-center p-4">
                    <div className="max-w-2xl w-full card-glass p-8 animate-scale-in">
                         ...
                         <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 mb-6">
                              <h3 className="font-semibold text-amber-800 dark:text-amber-300 text-sm mb-2 flex items-center gap-2">
                                   <HiExclamationTriangle className="w-4 h-4" /> Important Instructions
                              </h3>
                              <ul className="text-xs text-amber-700 dark:text-amber-400 space-y-1.5">
                                   <li>• The test will enter fullscreen mode. Do not exit fullscreen.</li>
                                   <li>• Switching tabs more than 2 times will auto-submit your test.</li>
                                   <li>• Right-click, copy, paste, and keyboard shortcuts are disabled.</li>
                                   <li>• The test will auto-submit when the timer runs out.</li>
                                   <li>• You can mark questions for review and navigate using the palette.</li>
     ...
     // Result screen — useSelector for `result` is now at component top level
     if (isSubmitted) {
          return (
               <div className="min-h-screen bg-surface-50 dark:bg-surface-950 flex items-center justify-center p-4">
                    <div className="max-w-lg w-full card-glass p-8 text-center animate-scale-in">
                         ...
                         <h1 className="text-2xl font-display font-bold text-surface-900 dark:text-white mb-2">Test Submitted!</h1>
                         <p className="text-surface-500 mb-8">Your responses have been recorded</p>
                         ...
                         <div className="grid grid-cols-2 gap-4 mb-8">
                              ...
                              <div className="p-4 rounded-xl bg-surface-50 dark:bg-surface-800/50">
                                   <p className={`text-3xl font-bold ${(result?.tabSwitchCount || 0) > 2 ? 'text-red-500' : 'text-surface-900 dark:text-white'}`}>
     ...
     return (
          <div className="min-h-screen bg-surface-50 dark:bg-surface-950 no-select">
                         ...
                         <div className="card p-6 lg:p-8">
                              ...
                              <div className="flex items-center justify-between mt-8 pt-6 border-t border-surface-100 dark:border-surface-800">
                                   ...
                                   <div className="flex items-center gap-2">
                                        ...
                                        <button
                                             onClick={() => dispatch(toggleMarkForReview(currentQ._id))}
                                             className={`btn text-sm gap-1 ${markedForReview.includes(currentQ._id)
                                                  ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800'
                                                  : 'btn-secondary'
                                                  }`}
                                        >
                                             <HiFlag className="w-4 h-4" />
                                             {markedForReview.includes(currentQ._id) ? 'Marked' : 'Mark'}
                                        </button>
                                        <button
               ...
               {/* Warning Modal on tab switch */}
               <Modal isOpen={showWarning} onClose={() => dispatch(setShowWarning(false))} title="⚠️ Warning: Tab Switch Detected" size="sm">
                    <div className="space-y-4">
                         <div className="p-4 rounded-xl bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800">
                              <p className="text-sm text-red-700 dark:text-red-300">
                                   Switching tabs during the exam is not allowed. If you switch tabs <strong>one more time</strong>, your test will be <strong>auto-submitted</strong> immediately.
                              </p>
                         </div>
                         <button onClick={() => dispatch(setShowWarning(false))} className="btn-primary w-full">
